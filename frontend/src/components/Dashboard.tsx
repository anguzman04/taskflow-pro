...
type View = 'tasks' | 'users' | 'areas' | 'control' | 'projects' | 'reports' | 'gantt' | 'changecontrol' | 'kpi';
...
export default function App() {
  ...
  const canViewKpi = currentUser?.is_admin || (currentUser as any)?.perm_kpi;
...
const handleDeleteEvidence = async (evidenceId: number) => {
  ...
  const handleUserSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const userData = {
        ...
        perm_kpi: formData.get('perm_kpi') === 'on' ? 1 : 0,
  ...
  const exportToExcelData = async (tasksToExport: any[], filenamePrefix: string, filterDesc?: string) => {
    ...
    const COLS = [
      ...
      { key: 'ultimoAvanceTexto',  width: 50, header: 'Comentario Último Avance' },
    ...
    tasksToExport.forEach((task, idx) => {
      ...
      const row = ws1.addRow({
        ...
        ultimoAvanceTexto: (task as any).ultimo_avance_texto || '-',
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 ${isSidebarCollapsed ? 'md:w-20' : 'md:w-64'} bg-white border-r border-slate-200 flex flex-col transform transition-all duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:sticky md:top-0 h-screen shrink-0 shadow-xl md:shadow-none`}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {canViewKpi && <button onClick={() => { setCurrentView('kpi'); if(window.innerWidth < 768) setIsSidebarOpen(false); }} title="Indicadores (KPI)" className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${isSidebarCollapsed ? 'md:justify-center md:px-0' : ''} ${currentView === 'kpi' ? 'bg-blue-50 text-blue-600' : 'text-slate-500 hover:bg-slate-50'}`}><TrendingUp size={18} className="shrink-0" /> <span className={isSidebarCollapsed ? 'md:hidden' : ''}>Indicadores (KPI)</span></button>}
      ...
      <main className="flex-1 overflow-y-auto h-screen relative" onClick={() => { if(showImportMenu) setShowImportMenu(false); if(showColumnManager) setShowColumnManager(false); }}>
              ...
              <div className="flex justify-between items-center gap-4">
                <div className="min-w-0 flex items-center gap-3">
                  ...
                  {currentView === 'tasks' ? (
                    ...
                    <>
                      <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 capitalize truncate">Indicadores (KPI)</h2>
                      <p className="text-slate-500 text-xs sm:text-sm truncate">Registro de eventos y cálculo de métricas SAPP</p>
                    </>
                  ) : (
                    <>
        ...
        <div className="p-4 lg:p-8">
          ...
          {currentView === 'kpi' && <KpiView currentUser={currentUser} />}
                  ...
                  <>
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                         ...
                         <div className="space-y-2 p-4 border border-slate-200 rounded-xl bg-white shadow-sm"><h5 className="text-xs font-bold text-slate-500 uppercase pb-2 border-b border-slate-100">Indicadores (KPI)</h5><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="perm_kpi" defaultChecked={(editingItem as any)?.perm_kpi} className="rounded text-indigo-600" /> Ver / Registrar</label></div>

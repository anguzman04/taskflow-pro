...
export default function App() {
  ...
  const globalProgress = currentView === 'tasks' 
    ? (filteredTasks.length > 0 ? (filteredTasks.reduce((acc, t) => acc + t.porcentaje_avance, 0) / filteredTasks.length) : 0)
    : (controlTasks.length > 0 ? (controlTasks.reduce((acc, t) => acc + t.porcentaje_avance, 0) / controlTasks.length) : 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="flex items-end gap-2">
              <span className={`text-3xl font-black ${getProgressTextColor(globalProgress)}`}>{globalProgress.toFixed(1)}%</span>
                  ...
                  <BarChart data={barData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Bar dataKey="Avance" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={50}>
                      {barData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.Avance >= 90 ? '#10b981' : entry.Avance >= 70 ? '#3b82f6' : entry.Avance >= 40 ? '#f59e0b' : '#ef4444'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                  ...
                  {(currentView === 'tasks' || currentView === 'control') && (
                    <div className="hidden lg:flex bg-slate-50 rounded-xl px-4 py-2 border border-slate-200 items-center gap-4">
                      <div className="text-right">
                        ...
                        <div className={`text-sm font-bold ${getProgressTextColor(globalProgress)}`}>
                          {globalProgress.toFixed(1)}%
                        </div>
                      </div>
                      <div className="w-24 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                        <div className={`${getProgressColor(globalProgress)} h-full transition-all duration-500`} style={{ width: `${globalProgress}%` }} />
                      </div>
                    </div>
                  )}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-16 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                              <div className={`${getProgressColor(task.porcentaje_avance)} h-full transition-all`} style={{ width: `${task.porcentaje_avance}%` }} />
                            </div>

...
window.fetch = async (...args) => {
  ...
  if (typeof resource === 'string' && resource.startsWith('/api')) {
    config = config || {};
    const isFormData = config.body instanceof FormData; // ¿Es un archivo?
    
    config.headers = {
      ...config.headers,
      'Authorization': `Bearer ${localStorage.getItem('token')}`,
      ...(isFormData ? {} : { 'Content-Type': 'application/json' }) // Si es archivo, NO fuerza el JSON
    };
...
export default function App() {
  ...
  // --- FUNCIÓN HELPER PARA RESOLVER NOMBRES DE ÁREAS ---
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Progreso Real</span>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg"><Eye size={18} /></div>
